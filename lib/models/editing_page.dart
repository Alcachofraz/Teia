...
class EditingPage {
  ...
  final List<Letter> snippets;
  final Map<int, bool> depths;

  static const int intMaxValue = 9223372036854775807;
  static const int boundary = 10;

  EditingPage(this.id, this.chapterId, this.storyId, this.snippets, this.depths);

  List<int> generateId(List<int>? p, List<int>? q) {
    if (p != null && q != null) {
    } else if (p != null && q == null) {
      List<int> ret = List.from(p);
      if (intMaxValue - boundary > ret.last) {
        ret.last += boundary;
      } else {
        ret.add(0);
      }
      ...
      return List.from(q)..add(0); // Create new depth
    } else {
      // All null
      return [0]; // Create first id
    }
  }
