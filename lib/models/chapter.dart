...
class Chapter extends Model {
  ...
  const Chapter._internal({required this.id, required title, graph, story, pages, createdAt, updatedAt, chapterGraphId}): _title = title, _graph = graph, _story = story, _pages = pages, _createdAt = createdAt, _updatedAt = updatedAt, _chapterGraphId = chapterGraphId;
  
  factory Chapter({String? id, required String title, ChapterGraph? graph, Story? story, List<Page>? pages, String? chapterGraphId}) {
  ...
  Map<String, dynamic> toJson() => {
    'id': id, 'title': _title, 'graph': _graph?.toJson(), 'story': _story?.toJson(), 'pages': _pages?.map((Page? e) => e?.toJson()).toList(), 'createdAt': _createdAt?.format(), 'updatedAt': _updatedAt?.format(), 'chapterGraphId': _chapterGraphId
  };
  
  Map<String, Object?> toMap() => {
    'id': id, 'title': _title, 'graph': _graph, 'story': _story, 'pages': _pages, 'createdAt': _createdAt, 'updatedAt': _updatedAt, 'chapterGraphId': _chapterGraphId
  };

  static final QueryModelIdentifier<ChapterModelIdentifier> MODEL_IDENTIFIER = QueryModelIdentifier<ChapterModelIdentifier>();
  static final QueryField ID = QueryField(fieldName: "id");
  static final QueryField TITLE = QueryField(fieldName: "title");
  static final QueryField GRAPH = QueryField(
    fieldName: "graph",
    fieldType: ModelFieldType(ModelFieldTypeEnum.model, ofModelName: 'ChapterGraph'));
  static final QueryField STORY = QueryField(
    fieldName: "story",
    fieldType: ModelFieldType(ModelFieldTypeEnum.model, ofModelName: 'Story'));
  static final QueryField PAGES = QueryField(
    fieldName: "pages",
  ...
  static var schema = Model.defineSchema(define: (ModelSchemaDefinition modelSchemaDefinition) {
    ...
    modelSchemaDefinition.addField(ModelFieldDefinition.hasOne(
      key: Chapter.GRAPH,
      isRequired: false,
      ofModelName: 'ChapterGraph',
      associatedKey: ChapterGraph.CHAPTER
    ));
    
    modelSchemaDefinition.addField(ModelFieldDefinition.belongsTo(
      key: Chapter.STORY,
      isRequired: false,
      targetNames: ['storyChaptersId'],
      ofModelName: 'Story'
    ));
